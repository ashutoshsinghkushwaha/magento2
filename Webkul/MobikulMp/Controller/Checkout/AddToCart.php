...
namespace Webkul\MobikulMp\Controller\Checkout;

use Webkul\MobikulApi\Controller\Checkout\AbstractCheckout;

/**
 * Class AddToCart conreoller
 */
class AddToCart extends AbstractCheckout
{
    ...
    public function execute()
    {
        ...
        try {
            if ($this->wholeData) {
                ...
                // Product limit check ////////////////////////////////
                $mpProductCartLimit = $this->checkAndUpdateProductCartLimit($product->getSku());
                if ($mpProductCartLimit && $mpProductCartLimit != "" && $mpProductCartLimit < $this->qty) {
        ...
        if ($product->getTypeId() == "downloadable") {
            ...
            if (isset($this->params["super_attribute"])) {
                $request = [
                    ...
                    "options"=>$paramOption, "product"=>$this->productId
                ];
            }
     ...
     * @return bool|float
     */
    private function checkAndUpdateProductCartLimit(string $sku)
    {
        try {
            //Object manager for MobikulMp Helper class//
            $marketplaceHelperData = \Magento\Framework\App\ObjectManager::getInstance()->get(
                \Webkul\Marketplace\Helper\Data::class
            );
            $allowProductLimit = $marketplaceHelperData->getAllowProductLimit();
            if ($allowProductLimit) {
                $product = $this->productRepository->get($sku);
                $sellerProductDataColl = $marketplaceHelperData->getSellerProductDataByProductId(
                    $product->getId()
                );
                if (count($sellerProductDataColl)) {
                    ...
                    if ($productTypeId != 'downloadable' && $productTypeId != 'virtual') {
                        ...
                        if (!$mpProductCartLimit) {
                            $mpProductCartLimit = $marketplaceHelperData->getGlobalProductLimitQty();
                        }
            ...
            $marketplaceHelperData->logDataInLogger(
                "IsCorrectQtyCondition checkAndUpdateProductCartLimit : ".$e->getMessage()
            );
        }
        return false;
    }
