...
/**
 * Class SendTrackingInfo controller
 * @author   Webkul <support@webkul.com>
 * @license  https://store.webkul.com/license.html ASL Licence
 * @link     https://store.webkul.com/license.html
 */
class SendTrackingInfo extends AbstractMarketplace
{
    /**
     * Execute function for class SendTrackingInfo
     *
    ...
    public function execute()
    {
        try {
            ...
            if ($shipmentDetails['success']) {
                ...
                $this->shipmentManager->notify($shipment->getEntityId());
                $this->returnArray["success"] = true;
                $this->returnArray["message"] = __("The message has been sent.");
            } else {
                $this->returnArray["message"] = $shipmentDetails['success'] ?? __("Failed to send the Shipping email.");
            }
