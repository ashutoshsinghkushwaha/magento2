...
class ViewCreditMemo extends AbstractMarketplace
{
    ...
    public function execute()
    {
        try {
            ...
            foreach ($creditmemo_items as $_item) {
                $itemList = $this->getItemList($_items, $orderId, $_item, $itemList, $paymentCode, $order);
            }
    ...
    public function getBundleProductItems($_ordereditem, $orderId, $_item, $eachItem, $paymentCode, $order)
    {
    ...
    public function getItemList($_items, $orderId, $_item, $itemList, $paymentCode, $order)
    {
        foreach ($_items as $_ordereditem) {
            ...
            if ($_ordereditem->getProductType()!='bundle') {
                ...
                $eachItem = $this->getProductItems($_ordereditem, $orderId, $_item, $eachItem, $paymentCode, $order);
            } else {
                // for bundle product
                $eachItem = $this->getBundleProductItems(
                    $_ordereditem, $orderId, $_item, $eachItem, $paymentCode, $order
                );
            }
            if (!empty($eachItem)) {
                $itemList[] = $eachItem;
